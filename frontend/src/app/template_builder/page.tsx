...
export default function TemplateBuilderDashboard() {
  ...
  async function loadTemplates() {
    ...
    try {
      ...
      // We could filter to show only templates created by the builder, 
      // but for now, we'll show all templates.
  ...
  const handleCreateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/template_builder/${res.id}/edit`);
    } catch (err: any) {
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-foreground/50 mt-1 text-sm">Design drag-and-drop email templates visually.</p>
        </div>
        <div className="flex w-full sm:w-auto">
          <Button onClick={() => setShowCreateModal(true)} className="w-full sm:w-auto flex items-center justify-center gap-2">
            <Plus size={16} />
            <span>Create Visual Template</span>
          </Button>
        </div>
      </div>
          ...
          {filteredTemplates.map(template => (
            <Card key={template.id} className="p-4 relative group flex flex-col justify-between hover:border-foreground/20 transition-colors">
              ...
              <div className="mt-6 flex items-center justify-between border-t border-border pt-3">
                ...
                <Link href={`/template_builder/${template.id}/edit`}>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-md p-6 border border-border bg-background shadow-lg relative">
            ...
            <h2 className="text-xl font-bold mb-4">Create Visual Template</h2>
