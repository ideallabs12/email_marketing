...
export default function AdvanceCampaignSendAnalyticsPage({ params }: { params: Promise<{ id: string, sendId: string }> }) {
  ...
  return (
    <div className="space-y-7">
      <div>
        <Link href={`/advance-campaigns/${advanceCampaignId}`} className="inline-flex items-center gap-1 text-sm text-foreground/55 hover:text-foreground mb-5">
          <ArrowLeft size={16} /> Back to advanced campaign
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Blast Analytics</h1>
            <p className="text-foreground/50 mt-1 text-sm">{analytics?.campaign.name || 'Loading blast…'}</p>
          </div>
